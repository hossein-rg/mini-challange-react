...
class FinishingUp extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      formYearOrMonth: this.props.plan[1],
    };
  }

  // eslint-disable-next-line react/prop-types
  formSelectPlan = this.props.plan[0];
  formSelectPlanPrice = 9;
  // eslint-disable-next-line react/prop-types
  formAddOns = [this.props.addons[0], this.props.addons[1], this.props.addons[2]];
  dataAddOns = [
    {
      dt: "Online service",
      mo: "+$1/mo",
  ...
  calcTotalPrice = () => {
    ...
    this.formAddOns.map((value, i) => {
      if (this.state.formYearOrMonth) value ? (total += this.dataAddOns[i].moPrice) : 0;
      else value ? (total += this.dataAddOns[i].yrPrice) : 0;
    });
      ...
      <div className={styles.div_finishingUp}>
        ...
        <div className={styles.box_finishingUp}>
          <div className={styles.box_finishingUp_changePlan}>
            ...
            <p>
              ${this.state.formYearOrMonth ? this.formSelectPlanPrice : this.formSelectPlanPrice * 10}/
              {this.state.formYearOrMonth ? "mo" : "yr"}
            </p>
          </div>
        ...
        <div className={styles.box_finishingUp_totalValue}>
          ...
          <span>
            ${this.calcTotalPrice()}/{this.state.formYearOrMonth ? "mo" : "yr"}
          </span>
        </div>
      </div>
    );
